...
func TestInfo(t *testing.T) {
	...
	assert.Equal("  Uptime: 15", lines[6])
	assert.Equal("  Mem alloc: 773552", lines[7])
	assert.Equal("  Hostname: localhost.localdomain", lines[8])
	assert.Equal("  Receiver host: localhost", lines[9])
	assert.Regexp(regexp.MustCompile(`^  Receiver port: \d+`), lines[10])
	assert.Equal("  Statsd host: localhost", lines[11])
	assert.Equal("  Statsd port: 8125", lines[12])
	assert.Equal("  API Endpoints: https://trace.agent.datadoghq.com", lines[13])
	assert.Equal("", lines[14])
	assert.Equal("  Spans received (1 min): 100", lines[15])
	assert.Equal("  Traces received (1 min): 10", lines[16])
	assert.Equal("  Spans dropped (1 min): 1", lines[17])
	assert.Equal("  Traces dropped (1 min): 3", lines[18])

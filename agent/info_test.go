...
func TestInfo(t *testing.T) {
	...
	assert.Regexp(regexp.MustCompile(`^  Command line: .*agent`), lines[4])
	assert.Equal("  Pid: 38149", lines[5])
	assert.Equal("  Uptime (seconds): 15", lines[6])
	assert.Equal("  Mem alloc (bytes): 773552", lines[7])
	assert.Equal("", lines[8])
	assert.Equal("  Hostname: localhost.localdomain", lines[9])
	assert.Equal("  Receiver: localhost:8126", lines[10])
	assert.Equal("  API Endpoints: https://trace.agent.datadoghq.com", lines[11])
	assert.Equal("", lines[12])
	assert.Equal("  Bytes received (traces, 1 min): 10000", lines[13])
	assert.Equal("  Traces received (1 min): 240", lines[14])
	assert.Equal("  Spans received (1 min): 360", lines[15])
	assert.Equal("", lines[16])
	assert.Equal("  Bytes sent (traces, 1 min): 3245", lines[17])
	assert.Equal("  Traces sent (1 min): 6", lines[18])
	assert.Equal("  Stats sent (1 min): 60", lines[19])
...
func TestWarning(t *testing.T) {
	...
	assert.Equal("  Traces received (1 min): 240", lines[14])
	assert.Equal("  Spans received (1 min): 360", lines[15])
	assert.Equal("  WARNING: Traces dropped (1 min): 5", lines[16])
	assert.Equal("  WARNING: Spans dropped (1 min): 10", lines[17])

...
func TestLogOnPanicMain(t *testing.T) {
	...
	defer func() {
		...
		assert.Contains(msg,
			"github.com/DataDog/datadog-trace-agent/internal/watchdog.TestLogOnPanicMain",
...
func TestLogOnPanicGoroutine(t *testing.T) {
	...
	go func() {
		defer func() {
			...
			assert.Contains(msg,
				"github.com/DataDog/datadog-trace-agent/internal/watchdog.TestLogOnPanicGoroutine",
				"log should contain a reference to this test func name as it displays the stack trace")
			wg.Done()
		}()
		defer LogOnPanic()

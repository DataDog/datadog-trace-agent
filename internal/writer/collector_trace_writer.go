...
import (
	...
	"github.com/DataDog/datadog-trace-agent/internal/collector"
...
func NewCollectorTraceWriter(conf *config.AgentConfig, in <-chan *agent.Trace) *CollectorTraceWriter {
	writerConf := conf.TraceWriterConfig
	log.Infof("Collector trace writer initializing with config: %+v", writerConf)
	endpoint := NewCollectorEndpoint(conf.CollectorConfig.CollectorAddr)
...
func (w *CollectorTraceWriter) handleTrace(trace *agent.Trace) {
...
func (w *CollectorTraceWriter) appendTrace(trace *agent.Trace) {

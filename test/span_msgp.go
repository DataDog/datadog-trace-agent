...
func (z *Span) Msgsize() (s int) {
	s = 1 + 8 + msgp.Uint64Size + 9 + msgp.Uint64Size + 10 + msgp.Uint64Size + 6 + msgp.Int64Size + 9 + msgp.Int64Size + 5 + msgp.MapHeaderSize
	if z.Meta != nil {
		for za0001, za0002 := range z.Meta {
			_ = za0002
	...
	s += 6 + msgp.Int32Size + 5 + msgp.StringPrefixSize + len(z.Name) + 8 + msgp.StringPrefixSize + len(z.Service) + 9 + msgp.StringPrefixSize + len(z.Resource) + 5 + msgp.StringPrefixSize + len(z.Type)
	return
}
...
// DecodeMsg implements msgp.Decodable
func (z *TraceList) DecodeMsg(dc *msgp.Reader) (err error) {
	var zb0003 uint32
...
// EncodeMsg implements msgp.Encodable
func (z TraceList) EncodeMsg(en *msgp.Writer) (err error) {
	err = en.WriteArrayHeader(uint32(len(z)))
	if err != nil {
		return
	}
...
// Msgsize returns an upper bound estimate of the number of bytes occupied by the serialized message
func (z TraceList) Msgsize() (s int) {
	s = msgp.ArrayHeaderSize
	for zb0005 := range z {
		s += msgp.ArrayHeaderSize
		for zb0006 := range z[zb0005] {
